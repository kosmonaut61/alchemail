...
5. **CRITICAL STATISTICS VALIDATION**: Use ONLY the exact statistics provided in the context repository. DO NOT create, fabricate, or invent any specific percentage numbers, dollar amounts, or time savings that are not explicitly listed in the verified context. If no specific statistics are available, use general descriptive language instead of specific numbers.
6. EVERY EMAIL MUST HAVE AN APOLLO LINK CTA that flows naturally in the sentence - can be anywhere in the email, format as [CTA text](https://app.apollo.io/#/meet/managed-meetings/{{sender_meeting_alias}}/n9l-1si-q4y/30-min)
   CRITICAL: Apollo links MUST use {{sender_meeting_alias}} (with underscore) - NEVER use {{sender.meeting.alias}} (with dot)
7. Be creative and compelling - focus on the signal story first, word count will be optimized later
8. FOCUS ON 1-2 SPECIFIC PAIN POINTS per email - avoid repeating the same pain point phrases across emails
9. NEVER use generic pain point phrases like "steep learning curve" in multiple emails
10. SINGLE PAIN POINT VARIATION: If only 1 pain point exists, use different angles per email:
   - Email 1: Core challenge
   - Email 2: Related aspects
   - Email 3: Impact/consequences
   - Email 4: Solutions/outcomes
11. CRITICAL: VARY PAIN POINT PHRASING - Use different ways to express the same concept without repeating phrases across emails
12. CRITICAL MERGE FIELD SYNTAX: Always use {{#endif}} to close conditionals, NEVER use {{/if}}
